...
class DenseGenerator(nn.Module):
    ...
    def __init__(self, config, input_dim, output_dim):
        ...
        self.projected_source_embedding_dim = config.projected_source_embedding_dim
        self.weight_generator = nn.Sequential(
            linear_layer(self.projected_source_embedding_dim, input_dim * output_dim))
        self.bias_generator = nn.Sequential(
            linear_layer(self.projected_source_embedding_dim, input_dim))

    def forward(self, input):
        return self.weight_generator(input).view(self.input_dim, self.output_dim), \
...
class LayerNormGenerator(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projected_source_embedding_dim = config.projected_source_embedding_dim
        self.weight_generator = linear_layer(self.projected_source_embedding_dim, config.input_dim)
        self.bias_generator = linear_layer(self.projected_source_embedding_dim, config.input_dim)
