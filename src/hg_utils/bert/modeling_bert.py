...
from src.adapters.adapter_modeling import MetaAdapterController
from src.adapters.source_modeling import SourceController
from .configuration_bert import BertConfig
from src.utils.dependency_parsing_utils import Biaffine, DependencyParsingOutput
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        # M2 adapters via hypernetwork(s)
        if self.hypernet:
            hidden_states = self.hypernet(self.source_embedding, hidden_states)

        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states

    def set_hypernet(self, hypernet, source_emb):
        self.hypernet = hypernet
        self.source_embedding = source_emb
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        if not self.config.no_hypernet:
            if config.condition_to_layer_id:
                self.hypernet = MetaAdapterController(config)
            else:
                self.hypernet = nn.ModuleList(MetaAdapterController(config) for _ in range(config.num_hidden_layers))

            self.hypernet_source_controller = SourceController(config)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # M2 adapters via hypernetwork(s)
            if not self.config.no_hypernet and not self.config.after_layer_output:
                if self.config.condition_to_layer_id:
                    source_embedding = self.hypernet_source_controller(task_name, lang_name, f'layer:{i}')
                    layer_module.output.set_hypernet(self.hypernet, source_embedding)
                else:
                    source_embedding = self.hypernet_source_controller(task_name, lang_name)
                    layer_module.output.set_hypernet(self.hypernet[i], source_embedding)
            ...
            # M2 adapters via hypernetwork(s)
            if not self.config.no_hypernet and self.config.after_layer_output:
                if self.config.condition_to_layer_id:
                    source_embedding = self.hypernet_source_controller(task_name, lang_name, f'layer:{i}')
                    hidden_states = self.hypernet(source_embedding, hidden_states)
                else:
                    source_embedding = self.hypernet_source_controller(task_name, lang_name)
                    hidden_states = self.hypernet[i](source_embedding, hidden_states)
...
class BertForDependencyParsing(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.biaffine_rels = Biaffine(n_in=config.hidden_size, n_out=self.num_labels, bias_x=True, bias_y=True)
    ...
    def forward(
        ...
        sequence_output = self.dropout(sequence_output)
        word_outputs_deps = self._merge_subword_tokens(sequence_output, word_starts)
    ...
    def _merge_subword_tokens(self, subword_outputs, word_starts):
        ...
        for i in range(len(subword_outputs)):
            ...
            mask = starts.ne(self.loss_fn.ignore_index)
            starts = starts[mask]
    ...
    def _get_loss(self, arc_preds, rel_preds, labels_arc, labels_rel, loss_fn):
        ...
        mask = labels_arc.ne(loss_fn.ignore_index)
